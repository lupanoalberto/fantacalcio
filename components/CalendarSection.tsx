...
export default function CalendarSection({ selectedLeague }: Props) {
  ...
  const isApiLeague = useMemo(
    () =>
      ...
      selectedLeague.trim().length > 0 &&
      !isDbLeague,
    [selectedLeague, isDbLeague],
  );
    ...
    async function loadInitialData() {
      try {
        ...
        if (isApiLeague) {
          ...
          setMatches(
            normalized.filter(
              (m) => Number(m.matchday) === (uniqueDays[0] ?? 1),
            ),
          );
          return;
        }
        ...
        if (isDbLeague) {
          ...
          const mapped: UIMatch[] = ((fxData ?? []) as FixtureRowDb[]).map(
            (r) => {
    ...
    async function loadDayData() {
      try {
        ...
        if (isApiLeague) {
          ...
          setMatches(
            normalized.filter((m) => Number(m.matchday) === Number(day)),
          );
          return;
        }

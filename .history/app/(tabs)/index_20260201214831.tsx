...
export default function LeagueDashboardScreen() {
  ...
  const [liveByLeague, setLiveByLeague] = useState<Record<string, boolean>>({});
  const [checkedByLeague, setCheckedByLeague] = useState<
    Record<string, boolean>
  >({});

  const handleHasMatches = (league: string) => (has: boolean) => {
    setLiveByLeague((prev) => ({ ...prev, [league]: has }));
    setCheckedByLeague((prev) => ({ ...prev, [league]: true }));
  };

  const allChecked = LEAGUES.every((l) => checkedByLeague[l]);
  const hasAnyLiveMatch = LEAGUES.some((l) => liveByLeague[l]);
  ...
  const showNoLiveText = allChecked && !hasAnyLiveMatch;
    ...
    >
      ...
      >
        ...
        <View style={styles.section}>
          ...
          {/* ✅ Monta SEMPRE i componenti (loro ritornano null se vuoti) */}
          {LEAGUES.map((lg) => (
            <LiveMatchesSection
              key={lg}
              selectedLeague={lg}
              onHasMatches={handleHasMatches(lg)}
            />
          ))}

          {/* ✅ Scritta SOLO se tutte hanno risposto e nessuna ha match */}
          {showNoLiveText ? (
            <Text
              style={{

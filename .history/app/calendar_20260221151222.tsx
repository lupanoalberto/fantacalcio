...
export default function CalendarScreen() {
  ...
  const renderMatchdays = () => {
    return (
      <View key={day} style={{ flexDirection: "column", justifyContent: "flex-start", alignItems: "flex-start", gap: 12 }}>
        <TouchableOpacity
        ...
        <View style={{ flexDirection: "column", width: "100%", gap: 6 }}>
          {matchesByMatchday.map((match) => {
            ...
            if (status === "IN_PLAY" || status === "1H" || status === "2H") {
              timeLabel = match?.status.elapsed ? `${match.status.elapsed}'` : "LIVE";
              scoreColor = true;
            }

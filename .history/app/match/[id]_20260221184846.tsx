...
export default function MatchDetails() {
  ...
  const ratingByPlayerId = useMemo(() => {
    ...
    for (const teamBlock of playersStats ?? []) {
      ...
      for (const row of players) {
        ...
        const stats0 = Array.isArray(row?.statistics) ? row.statistics[0] : null;
  ...
  const getVoteStyle = (vote?: number) => {
    ...
    if (v >= 7) return { backgroundColor: colors.green, color: colors.background };
    if (v >= 6) return { backgroundColor: colors.yellow, color: colors.text };
    if (v >= 5) return { backgroundColor: colors.orange ?? colors.yellow, color: colors.text };
    return { backgroundColor: colors.red, color: colors.background };
  };
    ...
    >
      ...
      >
        ...
        <View style={styles.section}>
          ...
          {/* TAB */}
          <ScrollView style={styles.containerScroll} horizontal showsHorizontalScrollIndicator={false}>
            <TouchableOpacity
              style={[
                styles.tabBtn,
          ...
          <View style={{ flexDirection: isActive === 0 ? "column" : "row" }}>
            {isActive === 0 ? (
              eventsLoading ? (
                ...
                <Text style={{ color: colors.textSecondary, fontSize: 12, padding: 12 }}>
                    ...
                    {Array.isArray(l.startXI) && l.startXI.length > 0 ? (
                      <View style={{ width: "100%" }}>
                        {l.startXI.map((p: any, j: number) => {
                          ...
                          const vote = Number.isFinite(pid) ? ratingByPlayerId.get(pid) : undefined;
                          ...
                          return (
                            <View
                              ...
                              style={{
                                ...
                                justifyContent: "flex-start",
                                alignItems: isHome === 0 ? "flex-start" : "flex-end",
                                padding: 12,
                                borderBottomWidth: 1,
                                borderColor: colors.gray,
                                gap: 6,
                            ...
                            >
                              <View
                                style={{
                                  flexDirection: isHome === 0 ? "row" : "row-reverse",
                                  gap: 6,
                                  alignItems: "center",
                                  justifyContent: "flex-start",
                                  width: "100%",
                              ...
                              >
                                ...
                                >
                                  ...
                                  >
                                    {Number.isFinite(vote as number) ? (vote as number).toFixed(1) : "—"}
                                  </Text>
                                </View>
                              </View>
                              ...
                              >
                                ...
                                {chips.length === 0 ? null : chips.map((chip, k) => (
                                  <View
                                    key={`${chip.type}-${chip.detail ?? ""}-${k}`}
                                    style={{ flexDirection: "row", alignItems: "center", gap: 4 }}
                                  >
                                    <EventIcon type={chip.type} detail={chip.detail} size={18} />
                                    {chip.count > 1 ? (
                                      <Text
                                        style={{
                                          color: colors.textSecondary,

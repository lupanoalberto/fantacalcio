...
const mapped: FixtureRow[] = dbRows.map((r) => ({
  ...
  home_team: Array.isArray(r.home_team) ? r.home_team[0] ?? null : null,
  away_team: Array.isArray(r.away_team) ? r.away_team[0] ?? null : null,
}));
...
setFixtures(mapped);
      ...
      Alert.alert("Errore", e?.message ?? "Impossibile caricare il calendario.");
  ...
  useEffect(() => {
    ...
    async function loadFixturesForDay() {
      try {
        ...
        if (!cancelled) setFixtures((fxData ?? []) as FixtureRow[]);
      } catch (e: any) {
  ...
  async function handleCreateCalendar() {
    try {
      ...
      Alert.alert(
        "Errore",
        e?.message ?? "Impossibile creare il calendario.",
      );
    } finally {
      setCreating(false);
    }
            ...
            fixtures.map((fx) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.85}
                  onPress={() => {
                    // Se vuoi una pagina dettaglio matchday / fixture, la colleghi qui.
                    // router.push(`/leagues/${String(leagueId)}/calendar/${fx.id}` as Href);
                  }}
                  style={{
                    marginHorizontal: 12,
                    padding: 12,

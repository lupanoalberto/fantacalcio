...
function renderLineupColumn(params: {
  ...
  split: {
    starters: (LineupPlayerRow & { roster?: RosterRow; stat?: PlayerStatRow })[];
    bench: (LineupPlayerRow & { roster?: RosterRow; stat?: PlayerStatRow })[];
  };
  isMine: boolean;
}) {
      ...
      {lineup ? (
        <>
          ...
          {split.starters.length === 0 ? (
            ...
            split.starters.map((p) => <PlayerRow key={`s-${p.lineup_id}-${p.api_player_id}-${p.slot}`} p={p} />)
          )}
          ...
          {split.bench.length === 0 ? (
            ...
            split.bench.map((p) => <PlayerRow key={`b-${p.lineup_id}-${p.api_player_id}-${p.slot}`} p={p} />)
          )}
        </>
      ) : isMine ? (
        <TouchableOpacity
    ...
    >
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          >
            <View>
              {/* INSERISCI FORMAZIONE HOME, OPPURE IL BOTTONE "INSERISCI FORMAZIONE" SE è LA SQUADRA DELL'UTENTE LOGGATO, OPPURE LA SCRITTA "FORMAZIONE NON DISPONIBILE" */}
            </View>
            <View>
              {/* INSERISCI FORMAZIONE AWAY, OPPURE IL BOTTONE "INSERISCI FORMAZIONE" SE è LA SQUADRA DELL'UTENTE LOGGATO, OPPURE LA SCRITTA "FORMAZIONE NON DISPONIBILE" */}
            </View>
          </View>
        </View>
      </ScrollView>

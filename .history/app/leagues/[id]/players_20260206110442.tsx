...
export default function LeaguePlayersScreen() {
  ...
  const [selectedPlayer, setSelectedPlayer] = useState<CompetitionPlayer | null>(null);
...
const [loadingTaken, setLoadingTaken] = useState(false);
  ...
  const maxAffordableNow = Math.max(0, budgetRemaining - Math.max(0, remainingPlayers - 1));
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      try {
        ...
        const myTeamId =
          (rows as any).find((r: any) => String(r?.team?.league_id) === String(leagueId))?.team?.id ??
          null;

        if (!myTeamId) throw new Error("Non ho trovato la tua squadra in questa lega.");
  ...
  async function refreshBudgetAndRoster() {
    ...
    try {
      ...
      const spent = active.reduce((acc: number, r: any) => acc + (Number(r.price ?? 0) || 0), 0);
  ...
  const visiblePlayers = useMemo(() => {
    ...
    if (q.length > 0) {
      list = list.filter((p) => {
        ...
        const team = normalizeForSearch(p.team?.name ?? p.team?.shortName ?? "");
  ...
  async function addPlayerToMyTeam(player: CompetitionPlayer, price: number) {
    try {
      ...
      if (!Number.isFinite(price) || price < 1) {
        Alert.alert("Prezzo non valido", "Inserisci un prezzo minimo di 1 credito.");
        return;
      }
      ...
      if ((already ?? []).length > 0) {
        Alert.alert("Giocatore già presente", "Hai già questo giocatore in rosa.");
        return;
      }
      ...
      const { error } = await supabase.from("rosters").insert(payload);
      ...
      Alert.alert("Errore", e?.message ?? "Impossibile aggiungere il giocatore.");
    } finally {
      setAdding(false);
    }
  }
  ...
  const renderPlayerRow = (player: CompetitionPlayer) => {
    ...
    return (
      ...
      >
        <Image source={{ uri: photo }} style={{ width: 48, height: 48, borderRadius: 12 }} />
  ...
  const RoleButton = ({ label, role }: { label: string; role: CanonicalRole }) => {
    ...
    return (
      ...
      >
        <Text style={{ fontFamily: fonts.semibold, color: colors.text, fontSize: 11, textTransform: "uppercase" }}>
          {label}
        </Text>
      </TouchableOpacity>
    );
    ...
    <View style={{ flex: 1, backgroundColor: colors.background, paddingBottom: insets.bottom }}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ gap: 12, paddingBottom: 12 }}>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 16, fontFamily: fonts.semibold, color: colors.text }} numberOfLines={1}>
              {headerTitle}
            </Text>
            ...
            <Text
              ...
              numberOfLines={1}
            >
              Budget: {budgetRemaining}/{totalBudget} • Rosa: {playersTaken}/{rosterSize}
        ...
        {loading ? (
          ...
          <>
            ...
            <View style={{ paddingHorizontal: 12, gap: 12 }}>
              ...
              <Text style={{ color: colors.textSecondary, fontFamily: fonts.regular, fontSize: 10, textAlign: "left" }}>
                {visiblePlayers.length} giocatori
              </Text>
            </View>

            {/* LISTA */}
            <View style={{ flexDirection: "column" }}>{visiblePlayers.map(renderPlayerRow)}</View>
          </>
        )}
      </ScrollView>
      ...
      >
        <Pressable style={styles.overlay} onPress={() => setSelectedPlayer(null)} />
        <View style={[styles.sheet, { backgroundColor: colors.background }]}>
          <View style={styles.sheetHeader}>
            <Text style={{ color: colors.text, fontFamily: fonts.semibold, fontSize: 12 }}>Acquista giocatore</Text>
            <TouchableOpacity onPress={() => setSelectedPlayer(null)} hitSlop={10}>
          ...
          {selectedPlayer ? (
            <View style={{ paddingHorizontal: 12, paddingBottom: 12, gap: 12 }}>
              <View style={{ flexDirection: "row", gap: 12, alignItems: "center" }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={{ color: colors.textSecondary, fontFamily: fonts.regular, fontSize: 11 }}>
                    ...
                    {selectedPlayer.team?.shortName ?? selectedPlayer.team?.name ?? "—"}
                  </Text>
                </View>
              </View>
              ...
              <View style={{ gap: 8 }}>
                <Text style={{ color: colors.textSecondary, fontFamily: fonts.regular, fontSize: 11 }}>
                  Budget disponibile: {budgetRemaining} • Giocatori mancanti: {remainingPlayers} • Max ora:{" "}
                  {maxAffordableNow}
                </Text>
              ...
              <TouchableOpacity
                onPress={() => addPlayerToMyTeam(selectedPlayer, parsePrice(priceText))}
                activeOpacity={0.85}
                disabled={adding}
                style={{
                  padding: 14,

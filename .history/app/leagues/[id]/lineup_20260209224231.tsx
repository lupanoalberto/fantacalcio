...
export default function LineupWizardScreen() {
  ...
  const rosterIsComplete = useMemo(() => {
    ...
    const roles: CanonicalRole[] = ["Portiere", "Difensore", "Centrocampista", "Attaccante"];
  ...
  .from("user_team")
  .select("role, team:teams!inner(id, league_id)")
  .eq("user_id", uid)
  ...
  async function saveLineup() {
    try {
      ...
      if (bench.length !== BENCH_SIZE) {
        Alert.alert("Panchina incompleta", `Seleziona ${BENCH_SIZE} giocatori in panchina.`);
        return;
      }
        ...
        {loading ? (
          ...
          <View style={{ paddingHorizontal: 12, gap: 8 }}>
            <View style={[styles.box, { borderColor: colors.secondary }]}>
              ...
              <Text style={{ fontSize: 13, fontFamily: fonts.regular, color: colors.textSecondary, marginTop: 12 }}>
                Totale: {roster.length}/{REQUIRED_ROSTER_TOTAL}
              </Text>
              <Text style={{ fontSize: 13, fontFamily: fonts.regular, color: colors.textSecondary, marginTop: 4 }}>
                POR: {rosterCounts.Portiere}/{REQUIRED_BY_ROLE.Portiere} · DIF: {rosterCounts.Difensore}/{REQUIRED_BY_ROLE.Difensore}
              </Text>
              <Text style={{ fontSize: 13, fontFamily: fonts.regular, color: colors.textSecondary, marginTop: 4 }}>
                CEN: {rosterCounts.Centrocampista}/{REQUIRED_BY_ROLE.Centrocampista} · ATT: {rosterCounts.Attaccante}/{REQUIRED_BY_ROLE.Attaccante}
              </Text>
            </View>
          </View>
        ) : (
          <>
            ...
            <View style={{ paddingHorizontal: 12, gap: 12 }}>
                ...
                <View style={{ gap: 8 }}>
                  {!rosterByRole.gks.length ? (
                    ...
                    rosterByRole.gks.map((p) => (
                      <PlayerPick key={p.id} player={p} selected={gk?.id === p.id} onPress={() => setGk(p)} />
                    ))
                  )}
                </View>
              ) : null}
              ...
              {step === "DEF" && formationSpec ? (
                <View style={{ gap: 8 }}>
                  ...
                  {rosterByRole.d.map((p) => {
                    ...
                    const disabled = !selected && defs.length >= formationSpec.DEF;
                    return (
                      <PlayerPick
                        key={p.id}
                        player={p}
              ...
              {step === "MID" && formationSpec ? (
                <View style={{ gap: 8 }}>
                  ...
                  {rosterByRole.m.map((p) => {
                    ...
                    const disabled = !selected && mids.length >= formationSpec.MID;
                    return (
                      <PlayerPick
                        key={p.id}
                        player={p}
              ...
              {step === "FWD" && formationSpec ? (
                <View style={{ gap: 8 }}>
                  ...
                  {rosterByRole.f.map((p) => {
                    ...
                    const disabled = !selected && fwds.length >= formationSpec.FWD;
                    return (
                      <PlayerPick
                        key={p.id}
                        player={p}
              ...
              {step === "BENCH" ? (
                <View style={{ gap: 8 }}>
                  <Text style={{ fontSize: 13, fontFamily: fonts.regular, color: colors.textSecondary }}>
                    Seleziona {BENCH_SIZE} giocatori in panchina ({bench.length}/{BENCH_SIZE})
                  </Text>
              ...
              {step === "REVIEW" ? (
                <View style={{ gap: 8 }}>
                  <View style={[styles.box, { borderColor: colors.secondary }]}>
                    ...
                    <Text style={{ fontSize: 13, fontFamily: fonts.semibold, color: colors.text, marginTop: 12 }}>
                      Panchina ({bench.length}/{BENCH_SIZE})
                    </Text>
                    <Text style={{ fontSize: 13, fontFamily: fonts.regular, color: colors.textSecondary, marginTop: 8 }}>
                      {bench.map((x) => x.cached_display_name).filter(Boolean).join(", ") || "—"}
                    </Text>
                  </View>
                </View>
              ) : null}

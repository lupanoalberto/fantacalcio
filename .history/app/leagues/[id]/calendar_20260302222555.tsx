      ...
      >
        ...
        <View style={{ flexDirection: "column", width: "100%", gap: 8 }}>
          {fixtures.map((fix) => {
            ...
            const homeLogo =
              fix?.home_team?.name?.crest ||
              fix?.home_team?.name?.crestUrl ||
            ...
            return (
              <View key={fix?.id}>
                <TouchableOpacity
                  onPress={() =>
                    router.push(`leagues/${leagueId}/match/${fix?.id}` as Href)
                  }
                  style={{ borderRadius: 8, overflow: "hidden" }}
                >
                  <MatchCard
                    key={fix?.id}
                    homeTeam={fix?.home_team?.name ? fix?.home_team?.name : "-"}
                    awayTeam={fix?.away_team?.name ? fix?.away_team?.name : "-"}
                    scoreHome={scoreHome}
                    scoreAway={scoreAway}
                    time={timeLabel}
                    homeLogo={homeLogo}
      ...
      >
        ...
        >
          ...
          >
            ...
            {calendarExists ? <>{renderMatchdays()}</> : null}
          </ScrollView>
        </LinearGradient>
      </View>
    );

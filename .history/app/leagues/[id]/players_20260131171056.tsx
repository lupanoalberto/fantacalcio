...
export default function ListoneScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const tId = utData?.team?.league_id ?? null;
        if (!tId) {
          // importante: se non trovi team, vuol dire che non sei membro della lega
          Alert.alert("Non sei nella lega", "Non risulti iscritto a questa lega.");
          return;
        }
  ...
  return (
    <View style={{ flex: 1, padding: 12, gap: 12, backgroundColor: colors.background }}>
          ...
          return (
            ...
            >
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontFamily: fonts.regular, color: colors.textSecondary, fontSize: 11 }} numberOfLines={1}>
              ...
              >
                {disabled ? (
                  ...
                  <Text style={{ fontFamily: fonts.semibold, color: colors.primary, fontSize: 11 }}>
                    Aggiungi
                  </Text>
                )}
              </TouchableOpacity>

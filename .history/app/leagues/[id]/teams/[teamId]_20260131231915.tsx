...
type CanonicalRole = "Portiere" | "Difensore" | "Centrocampista" | "Attaccante" | "Altro";

function toCanonicalRole(role?: string | null): CanonicalRole {
  const r = (role ?? "").toLowerCase();
  if (r.includes("por") || r.includes("gk") || r.includes("goal")) return "Portiere";
  if (r.includes("dif") || r.includes("def")) return "Difensore";
  if (r.includes("cen") || r.includes("mid")) return "Centrocampista";
  if (r.includes("att") || r.includes("forw") || r.includes("strik")) return "Attaccante";
  return "Altro";
}
...
export default function TeamPlayersScreen() {
  ...
  const params = useLocalSearchParams<{ id?: string | string[]; teamId?: string | string[] }>();
    ...
    async function load() {
      try {
        ...
        if (!isUuidLike(leagueId) || !isUuidLike(teamId)) {
          throw new Error(`Parametri non validi: leagueId=${String(leagueId)} teamId=${String(teamId)}`);
        }
        ...
        const { data: p, error: pErr } = await supabase
          .from("team_players")
          .select("id,team_id,api_player_id,player_name,role,player_photo,price")
          .eq("team_id", teamId)
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 12 }}>
          ...
          <ScrollView
            ...
            contentContainerStyle={{ paddingHorizontal: 12, gap: 12, paddingVertical: 12 }}
          >
            ...
            <RoleChip label="Centrocampista" value={countByRole.Centrocampista} />

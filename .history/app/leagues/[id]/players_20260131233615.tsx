...
export default function LeaguePlayersScreen() {
  ...
  // Filtri
  const [roleFilter, setRoleFilter] = useState<CanonicalRole | "Tutti">("Tutti");
  ...
  const OptionRow = ({
    ...
    >
      <Text style={{ color: colors.text, fontFamily: fonts.regular, fontSize: 10 }}>
        {label}
      </Text>
  ...
  async function addPlayerToMyTeam(player: CompetitionPlayer) {
    try {
      ...
      if (!uid) {
        Alert.alert("Login richiesto", "Devi essere loggato per aggiungere giocatori.");
      ...
      const tId = (utData as any)?.[0]?.team?.id ?? (utData as any)?.[0]?.team?.[0]?.id ?? null;
      if (!tId) {
      ...
      const payload = {
  team_id: teamId,          // assicurati sia uuid della squadra
  api_player_id: player.id, // o quello che usi tu
  name: player.name,        // se la tabella lo richiede
  role: role,               // se la tabella lo richiede
  // price: ...
};

const { data, error } = await supabase
  .from("NOME_TABELLA_ROSA")
  .insert(payload)
...
if (error) {
  ...
  return;
      ...
      Alert.alert("Ok!", `${player.name} aggiunto (stub). Ora aggancia la query/RPC reale.`);
    ...
    >
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ gap: 12, paddingBottom: 12 }}>
        ...
        {loading ? (
          ...
          <>
            {/* FILTRI */}
            <View style={{ width: "100%", flexDirection: "column", marginHorizontal: 12, gap: 12 }}>
              <View
                style={{
                  width: "100%",
                  flexDirection: "row",
              ...
              >
                <FilterChip label="Ruolo" value={roleFilter} onPress={() => setIsRoleOpen(true)} />
                <FilterChip label="Squadra" value={teamLabel} onPress={() => setIsTeamOpen(true)} />
                <FilterChip
                  label="Ordine"
      ...
      {/* Sheet Ruolo */}
      <Sheet visible={isRoleOpen} title="Filtra per ruolo" onClose={() => setIsRoleOpen(false)}>
      ...
      {/* Sheet Squadra */}
      <Sheet visible={isTeamOpen} title="Filtra per squadra" onClose={() => setIsTeamOpen(false)}>
        <ScrollView style={{ maxHeight: 420 }} showsVerticalScrollIndicator={false}>
          {teamOptions.map((t) => (
            <OptionRow
              key={t.id}
      ...
      {/* Sheet Ordinamento */}
      <Sheet visible={isSortOpen} title="Ordina" onClose={() => setIsSortOpen(false)}>
        <OptionRow
          label="A → Z"
          active={sortDir === "asc"}
          onPress={() => {

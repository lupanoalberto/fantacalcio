    ...
    async function loadBase() {
      try {
        ...
        const rosterSize = Number(
          rules?.roster_size ?? (league as any)?.roster_size ?? DEFAULT_RULES.rosterSize,
        );
  ...
  async function saveLineup() {
    try {
      ...
      if (bench.length !== benchSize) {
        Alert.alert("Panchina incompleta", `Seleziona ${benchSize} giocatori in panchina.`);
        return;
      }
          ...
          <View style={{ paddingHorizontal: 12, gap: 8 }}>
            <View style={[styles.box, { borderColor: colors.secondary }]}>
              ...
              <Text style={{ fontSize: 13, fontFamily: fonts.regular, color: colors.textSecondary, marginTop: 4 }}>
                POR: {rosterCounts.Portiere}/{leagueRules.quotas.P} · DIF: {rosterCounts.Difensore}/{leagueRules.quotas.D}
              </Text>
              <Text style={{ fontSize: 13, fontFamily: fonts.regular, color: colors.textSecondary, marginTop: 4 }}>
                CEN: {rosterCounts.Centrocampista}/{leagueRules.quotas.C} · ATT: {rosterCounts.Attaccante}/{leagueRules.quotas.A}
              </Text>
            </View>
          </View>
        ) : (
          <>
            ...
            <View style={{ paddingHorizontal: 12, gap: 12 }}>
                ...
                <View style={{ gap: 8 }}>
                  {(Object.keys(FORMATIONS) as FormationKey[]).map((f) => {
                    ...
                    return (
                      ...
                      >
                        <Text style={{ fontSize: 13, fontFamily: fonts.semibold, color: colors.text }}>{f}</Text>
                        <Text style={{ fontSize: 13, fontFamily: fonts.regular, color: colors.textSecondary, marginTop: 4 }}>
              ...
              {step === "BENCH" ? (
                <View style={{ gap: 8 }}>
                  <Text style={{ fontSize: 13, fontFamily: fonts.regular, color: colors.textSecondary }}>
                    Seleziona {benchSize} giocatori in panchina ({bench.length}/{benchSize})
                  </Text>

...
export default function LeaguePlayersScreen() {
  ...
  async function refreshTakenInLeague() {
    ...
    try {
      ...
      // prendo rosters attivi di TUTTE le squadre della lega
      // join rosters -> teams per filtrare league_id
      const { data, error } = await supabase
        .from("rosters")
        .select(
          `
          api_player_id,
          team:teams!rosters_team_id_fkey (
            id,
            league_id
          )
        `,
      ...
      (data ?? []).forEach((r: any) => {
        if (String(r?.team?.league_id) !== String(leagueId)) return;
        const pid = Number(r?.api_player_id);
  ...
  const renderPlayerRow = (player: CompetitionPlayer) => {
    ...
    return (
      ...
      >
        ...
        {isTaken ? (
  <Text style={{ fontFamily: fonts.semibold, fontSize: 10, color: colors.textSecondary }}>
    PRESO
  </Text>
) : (

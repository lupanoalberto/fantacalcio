...
export default function CreateLeagueScreen() {
  ...
  // ✅ NUOVO: campionato scelto qui, non arriva da params
  ...
  async function onJoinLeague() {
    ...
    if (!Number.isFinite(leagueId) || leagueId <= 0) {
      Alert.alert("Codice lega", "Il codice deve essere un numero valido (es: 123).");
      return;
    }

    const tJoinTeam = joinTeamName.trim();
    if (tJoinTeam.length < 2) {
      Alert.alert("Nome squadra", "Inserisci un nome squadra di almeno 2 caratteri.");
      return;
    }

    const { data: sess } = await supabase.auth.getSession();
    if (!sess.session?.user?.id) {
      Alert.alert("Login richiesto", "Devi essere loggato per unirti ad una lega.");
  ...
  async function onCreate() {
    ...
    if (!selectedLeague) {
      Alert.alert("Campionato mancante", "Seleziona un campionato prima di continuare.");
      return;
    }
    ...
    const { data: sess } = await supabase.auth.getSession();
    if (!sess.session?.user?.id) {
      Alert.alert("Login richiesto", "Devi essere loggato per creare una lega.");
      router.push("/(auth)/login");
      return;
    }
    ...
    const scoring_json = {
      ...
      events: {
        ...
        goal_conceded_gk: { value: toNumberOr(ptsGkConceded, -1), enabled: true },
    ...
    if (!created?.league_id) {
      Alert.alert("Errore creazione lega", "Risposta non valida dal server (league_id mancante).");
      return;
    }
  ...
  const Field = ({
    ...
    <View style={{ opacity: disabled ? 0.45 : 1 }}>
      <Text style={[styles.sectionTitle, { color: colors.primary, fontFamily: fonts.semibold }]}>
        {label}
      </Text>
      <TextInput
        value={value}
    ...
    <View style={{ paddingVertical: 8, paddingHorizontal: 16 }}>
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
        <View style={{ flex: 1 }}>
          ...
          {hint ? (
            <Text style={[styles.sectionTitle, { color: colors.secondary, fontFamily: fonts.semibold, fontSize: 10 }]}>
              {hint}
            </Text>
          ) : null}
        </View>
  ...
  // ✅ opzioni campionati (label utente)
  const leagueOptions = ["Serie A", "Premier League", "LaLiga", "Bundesliga", "Ligue 1"];

  return (
    <View style={{ flex: 1, paddingBottom: insets.bottom }}>
      <ScrollView style={{ flex: 1 }}>
        {/* ✅ JOIN LEAGUE */}
        <View style={{ borderWidth: 1, borderRadius: 16, padding: 16, margin: 16 }}>
          <Text style={[styles.sectionTitle, { color: colors.primary, fontFamily: fonts.semibold }]}>
            Unisciti ad una lega
          </Text>
          <Text style={[styles.sectionTitle, { color: colors.primary, fontFamily: fonts.regular, fontSize: 10 }]}>
            Inserisci l&apos;ID della lega (codice) e unisciti come partecipante.
          </Text>
        ...
        <Text
          style={[
            styles.sectionTitle,
            { color: colors.primary, fontFamily: fonts.semibold, paddingHorizontal: 16, fontSize: 14 },
          ]}
        >
          Crea una nuova lega
        </Text>
        ...
        <Text
          style={[
            styles.sectionTitle,
            { color: colors.secondary, fontFamily: fonts.semibold, paddingHorizontal: 16, fontSize: 12 },
          ]}
        >
        ...
        <View style={{ paddingHorizontal: 16, paddingVertical: 8, gap: 8 }}>
          ...
          {/* 1) Nome squadra */}
          <Field label="Nome squadra" value={teamName} onChangeText={setTeamName} placeholder="La mia squadra" />

          {/* 2) Nome lega */}
          <Field label="Nome lega" value={leagueName} onChangeText={setLeagueName} placeholder="Nome della lega" />
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>
            ...
            >
              <Text style={[styles.sectionTitle, { color: colors.secondary, fontFamily: fonts.semibold }]}>
                Classico
              </Text>
            </Pressable>
          ...
          <View>
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.sectionTitle, { color: colors.secondary, fontFamily: fonts.regular, fontSize: 10 }]}>
                  Applica bonus/malus in base alla media voto del reparto difensivo.
                </Text>
              </View>
        ...
        <Text
          style={[
            styles.sectionTitle,
            { color: colors.primary, fontFamily: fonts.semibold, paddingHorizontal: 16, paddingTop: 8, fontSize: 14 },
          ]}
        >
          Regole
        </Text>
        ...
        <View style={{ paddingHorizontal: 16, paddingVertical: 8, gap: 8 }}>
          <Field label="Budget" value={budget} onChangeText={setBudget} placeholder="500" keyboardType="numeric" />
          <Field label="Dimensione rosa" value={rosterSize} onChangeText={setRosterSize} placeholder="25" keyboardType="numeric" />
          <Field
        ...
        <View style={{ flexDirection: "row", gap: 8, paddingHorizontal: 16, marginTop: 8 }}>
          <View style={{ flex: 1 }}>
            <Field label="P" value={quotaP} onChangeText={setQuotaP} keyboardType="numeric" />
          </View>
          <View style={{ flex: 1 }}>
            <Field label="D" value={quotaD} onChangeText={setQuotaD} keyboardType="numeric" />
          </View>
          <View style={{ flex: 1 }}>
            <Field label="C" value={quotaC} onChangeText={setQuotaC} keyboardType="numeric" />
          </View>
          <View style={{ flex: 1 }}>
            <Field label="A" value={quotaA} onChangeText={setQuotaA} keyboardType="numeric" />
          </View>
        </View>
        ...
        <View style={{ paddingHorizontal: 16, paddingVertical: 8, gap: 8 }}>
          ...
          <Field label="Autogol" value={ptsOwnGoal} onChangeText={setPtsOwnGoal} />
          <Field label="Gol subito (solo portiere)" value={ptsGkConceded} onChangeText={setPtsGkConceded} />
          <Field label="Porta inviolata (solo portiere)" value={ptsGkClean} onChangeText={setPtsGkClean} />
          <Field label="Rigore segnato" value={ptsPenScored} onChangeText={setPtsPenScored} />
          <Field label="Rigore sbagliato" value={ptsPenMissed} onChangeText={setPtsPenMissed} />
          <Field label="Rigore parato" value={ptsPenSaved} onChangeText={setPtsPenSaved} />
        </View>
        ...
        <SpecialRow label="Palo / Traversa" enabled={enWoodwork} onToggle={setEnWoodwork} value={ptsWoodwork} onChangeText={setPtsWoodwork} />

        <SpecialRow label="Rigore procurato" enabled={enPenWon} onToggle={setEnPenWon} value={ptsPenWon} onChangeText={setPtsPenWon} />

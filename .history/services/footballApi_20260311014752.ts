...
export function toApiFootballLeagueId(input: string | number): number {
  const raw = String(input ?? "").trim();
  if (!raw) {
    throw new Error("Missing league identifier.");
  }

  // 1) Se arriva già un id numerico API-Football ("135", 135), lo uso direttamente
  if (/^\d+$/.test(raw)) {
    return Number(raw);
  }

  // 2) Se arriva un nome UI ("Serie A") lo converto in codice legacy ("SA")
  const code = LEAGUE_CODES[raw] ?? raw;

  // 3) Se arriva un codice legacy ("SA"), lo converto in id API-Football
  const id =
    APIFOOTBALL_LEAGUE_ID[code as keyof typeof APIFOOTBALL_LEAGUE_ID];

  if (!id) {
    throw new Error(
      `Unsupported league: ${String(input)} (normalized=${String(code)})`,
    );
  }
...
export type PersonDetails = {
  ...
  // ✅ compat con UI vecchia (evita errori su number)
  number?: number | null;
};
...
// -------------
// Players stats (ratings per fixture)
// -------------
//
// Endpoint: /fixtures/players?fixture={id}
// Serve per recuperare games.rating (il "voto") per ogni player della partita.
...
export async function getFixturePlayersStats(
  fixtureId: number,
): Promise<any[]> {
  const url = `${APIFOOTBALL_BASE_URL}/fixtures/players?fixture=${fixtureId}`;
  const res = await fetch(url, { headers: getApiFootballHeaders() });
  const raw = await res.text();

  if (!res.ok) {
    console.error(
      `❌ Errore fetch fixture players ${fixtureId}:`,
      res.status,
      raw,
    );
    return [];
  ...
  const data = parseJsonSafe<FixturePlayersResponse>(raw, "fixture players");
  const err = extractApiErrors(data);
  if (err) {
    console.error("❌ API-FOOTBALL fixture players errors:", err);
    return [];
  }
...
export async function getApiFootballPlayerSeasonStats(params: {
  ...
  return {
    ...
    in: st.substitutes?.in ?? null,
    out: st.substitutes?.out ?? null,
  };
}
...
export async function getCurrentMatchday(
  ...
  const leagueId = toApiFootballLeagueId(leagueNameOrCode);
  const season = getDefaultSeasonYear(); // marzo 2026 -> 2025
  ...
  const url = `${APIFOOTBALL_BASE_URL}/fixtures?league=${leagueId}&season=${season}&from=${fromStr}&to=${toStr}`;
  const res = await fetch(url, { headers: getApiFootballHeaders() });
  const raw = await res.text();

  if (!res.ok)
    throw new Error(
      `API-FOOTBALL fixtures window failed: ${res.status} ${raw}`,
    );

  const data = parseJsonSafe<FixturesResponse>(raw, "fixtures window");
  const err = extractApiErrors(data);
  if (err) throw new Error(`API-FOOTBALL fixtures window errors: ${err}`);

  const list = data.response ?? [];

  const items = list
    .map((x) => {
      const md = roundToMatchday(x.league.round ?? null);
      return {
        md,
        status: x.fixture.status.short,
        kickoff: new Date(x.fixture.date).getTime(),
      };
    })
    .filter((x) => Number.isFinite(x.md as number));

  if (!items.length) return null;

  const isLive = (s: string) =>
    ["1H", "2H", "HT", "ET", "PEN"].includes(String(s ?? "").toUpperCase());
  const isNS = (s: string) => String(s ?? "").toUpperCase() === "NS";
  const isFT = (s: string) =>
    ["FT", "AET", "PEN"].includes(String(s ?? "").toUpperCase());

  // 1) LIVE
  const live = items
    .filter((x) => isLive(x.status))
    .sort((a, b) => a.kickoff - b.kickoff);
  if (live.length) return live[0].md!;

  // 2) prossime NS
  const upcoming = items
    .filter((x) => isNS(x.status))
    .sort((a, b) => a.kickoff - b.kickoff);
  if (upcoming.length) return upcoming[0].md!;

  // 3) fallback ultimo FT
  const finals = items
    .filter((x) => isFT(x.status))
    .sort((a, b) => b.kickoff - a.kickoff);
  if (finals.length) return finals[0].md!;

  return items.sort((a, b) => b.kickoff - a.kickoff)[0].md ?? null;
}

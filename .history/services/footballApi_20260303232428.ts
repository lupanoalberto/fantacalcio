...
export async function getApiFootballPlayerSeasonStats(params: {
  ...
  return {
    ...
    appearances: st.games?.appearances,
...
export async function getCurrentMatchday(leagueNameOrCode: string): Promise<number | null> {
  const leagueId = toApiFootballLeagueId(leagueNameOrCode);
  ...
  if (!res.ok) throw new Error(`API-FOOTBALL fixtures window failed: ${res.status} ${raw}`);
  ...
  const isLive = (s: string) => ["1H", "2H", "HT", "ET", "PEN"].includes(String(s ?? "").toUpperCase());
  const isNS = (s: string) => String(s ?? "").toUpperCase() === "NS";
  const isFT = (s: string) => ["FT", "AET", "PEN"].includes(String(s ?? "").toUpperCase());

  // 1) LIVE
  const live = items.filter((x) => isLive(x.status)).sort((a, b) => a.kickoff - b.kickoff);
  if (live.length) return live[0].md!;

  // 2) prossime NS
  const upcoming = items.filter((x) => isNS(x.status)).sort((a, b) => a.kickoff - b.kickoff);
  if (upcoming.length) return upcoming[0].md!;

  // 3) fallback ultimo FT
  const finals = items.filter((x) => isFT(x.status)).sort((a, b) => b.kickoff - a.kickoff);
